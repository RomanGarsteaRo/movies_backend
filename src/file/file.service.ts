...
export class FileService {
    ...
    getFilesInFolder(folderPath: string): Observable<{ fileName: string, path: string, size: string }[]> {
        return from(readdir(folderPath)).pipe(
            mergeMap(files => {
                const fileStatsObservables = files.map(file => {
                    const filePath = path.join(folderPath, file);
                    return from(this.statAsync(filePath)).pipe(
                        map(stats => ({
                            fileName: file,
                            path: filePath,
                ...
                return forkJoin(fileStatsObservables);
            }),
